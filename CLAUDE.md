...
**后端**: FastAPI + LangChain + DeepSeek + Pydantic | **前端**: Vue 3 + Vite | **搜索**: Tavily API | **编排**: 自定义状态机 (while + dict) | **可观测**: LangFuse (可选) + logging (控制台+文件双输出)
...
│   │   │   └── logging.py      # 日志系统 — setup_logging() 控制台+文件双输出按天轮转
│   │   ├── schemas/
│   │   │   ├── __init__.py
...
│   │   └── tracker/
│   │       ├── __init__.py
│   │       └── langfuse.py     # LangFuse 追踪 — @trace_agent 装饰器（无 Key 时退化为 logging）
│   ├── tests/
│   │   ├── __init__.py
...
LANGFUSE_PUBLIC_KEY=pk-lf-xxx    # 可选，不填则退化为 logging 输出
LANGFUSE_SECRET_KEY=sk-lf-xxx
LANGFUSE_HOST=https://cloud.langfuse.com
LOG_LEVEL=INFO                   # 可选：DEBUG|INFO|WARNING|ERROR，控制台输出级别
```
...
## 可观测：LangFuse + Logging

### LangFuse 追踪 (`tracker/langfuse.py`)

模块级开关：检查 `.env` 中是否有 LangFuse 的 public_key 和 secret_key，没有则退化为 logging 输出。

`@trace_agent("name")` 装饰器记录：Agent 名称、耗时、token 消耗、当前 step、revision_count、异常信息。`_langfuse_enabled` 为 False 时装饰器只通过 `logger.info/error` 输出，不影响任何功能。

`flush()` 在 `main.py` 的 `lifespan` shutdown 阶段调用，确保进程关闭前 LangFuse 缓冲区数据发送完成。
...
| 层 | 文件 | 记录内容 |
|------|------|------|
| 启动/关闭 | `main.py` | 服务启动、LangFuse 启用状态、服务关闭 |
| HTTP | `router.py` | 请求入口、参数校验失败、流水线完成 |
| 编排 | `orchestator.py` | step 切换、审核通过/降级、护栏命中 |
| Agent | `langfuse.py` | Agent 耗时+token（INFO）、异常（ERROR） |
| Agent | `base.py` | JSON 解析失败（WARNING + DEBUG 堆栈） |
| 搜索 | `planning.py` | Tavily 搜索成功（DEBUG）、失败（WARNING） |
| 搜索 | `review.py` | Tavily 事实核查成功/失败 |
| 护栏 | `compliance.py` | 合规检测命中条数+类别 |

配置：`setup_logging()` 在 `main.py` 的 `lifespan` 启动阶段调用。控制台输出 ≥`LOG_LEVEL` 环境变量（默认 INFO），文件输出所有级别到 `logs/app.log`，每天午夜轮转，保留 7 天。
...
1. **搜索失败静默处理**：Tavily 搜索失败时返回 `""` 不抛异常，是刻意设计的降级策略（搜索只是辅助，不能阻塞主流程）。搜索失败会通过 `logger.warning()` 记录日志。修改时注意保持此行为。
2. **test_e2e.py 的 skipif 条件**：不仅检查 key 是否存在，还检查是否为占位符（包含 `"your-deepseek-key"`），避免用占位符跑浪费调用的测试。
3. **日志文件轮转**：`logs/app.log` 每天午夜自动轮转，旧文件保留 7 天（`app.log.2026-05-21` 格式）。如果日志量异常增大，检查是否误设为 `LOG_LEVEL=DEBUG`。
