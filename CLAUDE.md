...
# Windows 若遇 gRPC 报错，加 export DOCKER_BUILDKIT=0 禁用 BuildKit
export DOCKER_BUILDKIT=0 && docker compose -f docker/docker-compose.yml up -d --build

# 多容器扩展（Nginx upstream 自动负载均衡轮询）
...
│   ├── nginx.conf              #   Nginx 反代配置（SPA路由+upstream负载均衡）
│   └── .dockerignore           #   构建排除规则
├── docs/                       # 需求文档 + 设计文档
```
